...
export default async function ReaderPage({ params }: { params: Promise<{ source: string; parts: string[] }> }) {
  ...
  return (
    <main>
      ...
      <section className="reader-shell mx-auto grid gap-3 px-4 pb-28 lg:grid-cols-[14rem_1fr] lg:pb-10" dir="rtl">
        <aside className="order-2 space-y-2 lg:order-1 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto lg:pr-1" dir="rtl">
          ...
          {!!tocRes.data.length && (
            <nav className="rounded-xl border border-line bg-[rgb(var(--sheet))]/80 p-3 shadow-sm" aria-label="Table of contents">
              <p className="mb-2 font-arabic font-semibold text-ink">الفهرس</p>
              <div className="space-y-1 font-arabic text-sm leading-6">
                {tocRes.data.map((item, i) => {
                  ...
                  return <Link key={`${item.title}-${i}`} href={href} className={`block rounded-lg px-2 py-1 hover:bg-ink/5 ${active ? "text-ink" : "text-muted"}`}>{item.title}</Link>;
                })}
              </div>
            </nav>
          )}
        ...
        <article className="book-sheet order-1 rounded-2xl border border-line p-5 sm:p-7 lg:order-2 lg:p-8">
          ...
          {page ? (
            <>
              <h1 className="font-arabic text-2xl font-bold leading-tight">{page.bookTitle || info?.title || page.label || "صفحة قراءة"}</h1>
