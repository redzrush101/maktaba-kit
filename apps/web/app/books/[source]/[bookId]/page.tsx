...
export default async function BookPage({ params, searchParams }: { params: Promise<{ source: string; bookId: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <main>
      <Header />
      <section className="mx-auto max-w-5xl px-4 pb-10" dir="rtl">
        <div className="rounded-2xl border border-line bg-[rgb(var(--sheet))]/80 p-5 shadow-sm">
          ...
          <h1 className="font-arabic text-3xl font-bold leading-tight">{book?.title || "كتاب"}</h1>
          {book?.author && <p className="mt-2 font-arabic text-lg text-muted">{book.author}</p>}
          ...
          {!!categories.length && <div className="mt-4 flex flex-wrap gap-2 font-arabic text-sm text-muted">{categories.map((category, i) => category.name ? <span key={`${category.name}-${i}`} className="rounded-full border border-line px-3 py-1">{category.name}</span> : null)}</div>}
        ...
        {volumes.length > 0 && (
          <section className="mt-4 rounded-2xl border border-line bg-paper/60 p-4">
            <h2 className="mb-3 font-arabic text-xl font-semibold">الأجزاء</h2>
        ...
        <section className="mt-4 rounded-2xl border border-line bg-paper/60 p-4">
          <h2 className="mb-3 font-arabic text-xl font-semibold">ابحث داخل الكتاب</h2>
          <SearchBox hiddenFields={{ bookId, volume: source === "eshia" ? volume : undefined }} defaultSource={source} placeholder="ابحث داخل هذا الكتاب..." />
        </section>

        <section className="mt-4 rounded-2xl border border-line bg-paper/60 p-4">
          <h2 className="mb-3 font-arabic text-xl font-semibold">الفهرس</h2>
          {tocRes.data.length ? (
            <div className="grid gap-2 md:grid-cols-2">
              {tocRes.data.map((item, i) => {
                ...
                return <Link key={`${item.title}-${i}`} href={href} className="rounded-xl border border-line/80 bg-[rgb(var(--sheet))]/60 p-3 font-arabic text-ink hover:bg-ink/5"><span>{item.title}</span>{item.page && <span className="mr-2 font-sans text-xs text-muted" dir="ltr">p. {item.page}</span>}</Link>;
              })}
            </div>
          ) : <p className="font-arabic text-muted">لا يوجد فهرس متاح.</p>}
        </section>
      </section>
    </main>
  );
